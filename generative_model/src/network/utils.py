...
def timestep_embedding(timesteps: torch.Tensor, dim: int, max_period: int = 10000, repeat_only: bool = False):
    """
...
class FeedForward(nn.Module):
    def __init__(self, dim: int, dim_out: Optional[int] = None, mult: int = 4, glu: bool = False, dropout: float = 0.):
        super().__init__()
...
class CrossAttention(nn.Module):
    def __init__(self, query_dim: int, context_dim: Optional[int] = None, heads: int = 8, dim_head: int = 64, dropout: float = 0.):
        super().__init__()
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        d_head: int,
        dropout: float = 0.,
        context_dim: Optional[torch.Tensor] = None,
        gated_ff: bool = True,
        mult_ff: int = 2,
    ):
        super().__init__()
